...
internal class GModMaterialGma : ResourceLoader<GModMount>
{
	...
	protected override object Load()
	{
		try
		{
			...
			var pbrProps = PseudoPbrFormats.ExtractProperties( vmt, _entry.Path );
			...
			return pbrProps.Format switch
			{
				...
				PbrFormat.MadIvan18 => CreateMadIvan18Material( materialName, pbrProps, vmt ),
	...
	/// </summary>
	private Material CreateMadIvan18Material( string name, ExtractedPbrProperties props, VmtFile vmt )
	{
	...
	private Material CreateEyeMaterial( string name, ExtractedPbrProperties props )
	{
		...
		if ( !string.IsNullOrEmpty( props.IrisTexturePath ) )
			material.Set( "g_tIris", LoadTexture( props.IrisTexturePath ) ?? Texture.White );
		else
			material.Set( "g_tIris", Texture.White );
		
		// g_tCornea - cornea normal/parallax map
		if ( !string.IsNullOrEmpty( props.CorneaTexturePath ) )
			material.Set( "g_tCornea", LoadTexture( props.CorneaTexturePath ) ?? DefaultNormal );
		else
			material.Set( "g_tCornea", DefaultNormal );
		
		// Eye parameters
		material.Set( "g_flDilationFactor", props.EyeDilation );
		material.Set( "g_flParallaxStrength", props.EyeParallaxStrength );
		material.Set( "g_flCorneaBumpStrength", props.EyeCorneaBumpStrength );
		material.Set( "g_flGlossiness", props.EyeGlossiness );
		material.Set( "g_flEyeballRadius", 0.5f );
